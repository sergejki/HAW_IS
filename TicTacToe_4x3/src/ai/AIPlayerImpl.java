		...
		for(int line=0; line<3; line++){
			for(int column=0; column<4;column++){
				if(getField(line,column).isFree()){
					int temp_mark = 0;
					if(isField(line,column-1) && getField(line,column-1).isFree()){temp_mark++;}
					if(isField(line,column+1) && getField(line,column+1).isFree()){temp_mark++;}
					if(isField(line-1,column-1) && getField(line-1,column-1).isFree()){temp_mark++;}
					if(isField(line-1,column+1) && getField(line-1,column+1).isFree()){temp_mark++;}
					if(isField(line+1,column-1) && getField(line+1,column-1).isFree()){temp_mark++;}
					if(isField(line+1,column+1) && getField(line+1,column+1).isFree()){temp_mark++;}
					if(isField(line-1,column) && getField(line-1,column).isFree()){temp_mark++;}
					if(isField(line+1,column) && getField(line+1,column).isFree()){temp_mark++;}
					
					//Oben-Bonus
					if(isField(line-1,column) && getField(line-1,column).isX()){temp_mark++;}
					if(isField(line+1,column) && getField(line+1,column).isX()){temp_mark++;}
					
					//Unten-Bonus
					if(isField(line,column-1) && getField(line,column-1).isX()){temp_mark++;}
					if(isField(line,column+1) && getField(line,column+1).isX()){temp_mark++;}
					
					//Diagonal-Bonus
					if(isField(line-1,column+1) && isField(line+1,column-1) && getField(line-1,column+1).isX()){temp_mark++;}
					if(isField(line-1,column-1) && isField(line+1,column+1) && getField(line-1,column-1).isX()){temp_mark++;}
					if(isField(line+1,column+1) && isField(line-1,column-1) && getField(line+1,column+1).isX()){temp_mark++;}
					if(isField(line+1,column-1) && isField(line-1,column+1)&& getField(line+1,column-1).isX()){temp_mark++;}
					
					//Mögliche Reihe -Bonus
					if(isField(line-1,column) && isField(line+1,column) && (getField(line-1,column).isX() || getField(line+1,column).isX())){temp_mark++;}
